...
function updateRoomList() {
    ...
    Object.keys(pendingRooms).forEach(roomNum => {
        roomSelect.innerHTML += `<option value="${roomNum}">Phòng ${roomNum}</option>`;
    });
}
...
function processPayment() {
    ...
    const room = pendingRooms[roomNumber];
    const total = room.roomRate + room.services.reduce((sum, service) => sum + service.price, 0);
...
function generateReportData(payments, startDate, endDate) {
    ...
    const serviceRevenue = payments.reduce((sum, p) => 
...
function showPaymentHistory() {
    ...
    document.getElementById('searchInput').addEventListener('input', updatePaymentHistoryTable);
    document.getElementById('sortSelect').addEventListener('change', updatePaymentHistoryTable);
}
...
function updatePaymentHistoryTable() {
    ...
    // Cập nhật bảng
    tableBody.innerHTML = filteredHistory.map(payment => {
        const serviceTotal = payment.services.reduce((sum, service) => sum + service.price, 0);
        return `
            <tr>
                ...
                <td>
                    <button class="detail-btn" onclick="showPaymentDetail('${payment.roomNumber}', '${payment.paymentDate}')">
...
function showPaymentDetail(roomNumber, paymentDate) {
    const payment = paymentHistory.find(p => 
        p.roomNumber === roomNumber && p.paymentDate === paymentDate
    );
    
    if (!payment) return;

    // Cập nhật thông tin vào modal
    document.getElementById('detailRoomNumber').textContent = `Phòng ${payment.roomNumber}`;
    document.getElementById('detailPaymentDate').textContent = formatDateTime(payment.paymentDate);
    ...
    document.getElementById('detailRoomCharge').textContent = formatCurrency(payment.roomRate);

    // Hiển thị danh sách dịch vụ
    const servicesList = document.getElementById('detailServicesList');
    servicesList.innerHTML = payment.services.map(service => `
    ...
    // Hiển thị tổng cộng
    document.getElementById('detailTotal').textContent = formatCurrency(payment.total);
